...
TODO : use mgpud vs mgpuf images here
...
*Predict Live*
image::media/web/live_predict_0.png[title='Anomaly - Plot', width=50%]

image::media/web/live_predict_1.png[title='Anomaly - Plot Prediction Result', width=50%]
...
+
image::media/web/live_predict_0.png[title='Live Predict - Model', width=50%]
...
==== Symetry Web Configurations
TODO: symetry-rest.txt

Symetry Web can be configured by editing the `/opt/symetry/symetry-web.txt` configuration file. Ensure that `-Drtlm.web.db=/opt/symetry/symetry-web.txt` is set in `/opt/jetty/start.ini`.
