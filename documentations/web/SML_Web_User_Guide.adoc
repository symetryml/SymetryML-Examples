...
_Clustering_ models attempt to group your data such that the instances within a particular
group are more similar to each other than to their counterparts in other groups.
These models are unsupervised and can be built in an online fashion.
...
image::media/web/anomaly_plot_0.png[title='Anomaly - Plot', width=25%]

image::media/web/anomaly_plot_1.png[title='Anomaly - Plot Prediction Result', width=50%]
...
+
image::media/web/live_predict_0.png[title='Live Predict - Model', width=25%]

2. Specify the sink topic for the model.
+
image::media/web/live_predict_1.png[title='Live Predict - Stream Select', width=50%]

3. Verify the data via the preview panel
+
image::media/web/live_predict_2.png[title='Live Predict - Preview', width=50%]

4. Select the output column for the model. You can optionally specify the quantiles to be displayed.
+
image::media/web/live_predict_3.png[title='Live Predict - Column Select', width=50%]

5. View the resulting output
+
image::media/web/live_predict_4.png[title='Live Predict - Result', width=50%]

Live polling can be disabled by right clicking on the topic name and selecting
*Stop Polling*
...
3. When choosing a stream topic, the topic names will be displayed in the following format : *TOPIC_NAME:NUM_PARTITIONS*.
Ensure the correct partitions have been selected by clicking *Kafka Options* button and verifying the prefilled *kafka.partitions* field.
+
image::media/web/kafka_partitions0.png[title='Stream Topic Browser', width=50%]
{nbsp}
+
image::media/web/kafka_partitions1.png[title='Stream Parameters', width=50%]
{nbsp}
4. Ensure your information is valid and click *Next*
5. Verify the stream in the preview panel. Click *Next* to continue.
6. In the final panel specify the correct type mapping and click *Finish*
...
image::media/web/stream_options.png[title='Stream Actions', width=25%]
{nbsp} +
...
image::media/web/p_advanced_pt.png[title='Advanced Options - Power Transformation', width=50%]
{nbsp} +
...
image::media/web/p_advanced_km.png[title='Advanced Options - KM', width=50%]
{nbsp}
...
To build a clustering model a project needs to be created with a specific
set of parameters.
These parameters can be enabled as part of the *Advanced Project Options* on the last page of the project wizard.

image::media/web/cluster_0.png[title='Advanced Options - Clustering', width=50%]
{nbsp}
...
| *Cluster # Centroids* | Optional | Default is *100*. This parameter control the number of centroid that are kept in real time on the data. Typically this number is much higher than the intended number of cluster - e.g. the target 'k' parameters
| *Cluster Warmup Period* | Optional | Default is *101*. This parameters control how many tuples of data must be seen before the initial real-time cluster will be constructed using the 'cluster_num_centroids' parameter as
the k number. Once the initial cluster is created it will just be updated in real-time with each new tuple/row of data. Note that *Cluster Warmup Period* needs to be greater than *Cluster # Centroids*, e.g. *Cluster # Centroids*=100 and *Cluster # Centroids*=101. If this condition is not meet an error will be raised by the software.
|=============================
...
image::media/web/cluster_build_0.png[title='Build Model - K-Means', width=50%]
{nbsp}

image::media/web/cluster_build_1.png[title='Build Model - K-Means Parameters', width=50%]
{nbsp}
...
1. First, select the *K-Means Select* option under the *Exploration* menu.
+
image::media/web/cluster_optimize_0.png[title='Optimize K-Means', width=50%]
{nbsp}
+
2. Select the appropriate range of clusters to evaluate.
+
image::media/web/cluster_optimize_1.png[title='Optimize K-Means Parameters', width=50%]
{nbsp}
+
3. You will be notified of the results once the *Optimize K-Means Job* is complete.
+
image::media/web/cluster_optimize_2.png[title='Optimize K-Means Job Done', width=50%]
{nbsp}
+
4. The result of the optimization process is a set of two graphics which allow
you to compare the *WSSSE* and the *Silhoutte Scores* across different models.
+
image::media/web/cluster_optimize_3.png[title='Optimize K-Means Result', width=50%]
{nbsp}
+
5. You can complete the process by choosing the optimal number of clusters and 
clicking on the *Build Model* button.
...
New in symetry 5.4 is the ability to simplify project creation with
AutoML.

AutoML streamlines various stages of the typical machine learning pipeline. 
Enabling this feature will automatically filter out redundant features, engineer new ones, and allow the project to focus on a spacific attribute. 
AutoML is meant as a precursor to  <<model-select>>. Utilizing the two steps in sequence allows the user to quickly build up models with minimal data preprocessing.

image::media/web/automl_0.png[title='Update Fusion', width=50%]
...
{nbsp}
+
4.  After completing the model-building process, double click on the model icon to see information about your new model.
...
Most of the SML models allow for model specific options to be enabled before building. One example of such option is the *psuedo matrix inverse*. With *pseudo inverse* enabled, a linear model, which would typically not be build due to matrix non invertability, can be created by the user. However, such model is likely to be unstable and therefore this option should only be used by advanced users.

image::media/web/advanced_model_features_0.png[title='Model - Advanced Options', width=50%]

image::media/web/advanced_model_features_1.png[title='Model - Advanced Options LDA', width=50%]
...
[[symetry-web-configurations]]
==== Symetry Web Configurations

Certain Symetry Web parameters can be configured by editing the `/opt/symetry/symetry-web.txt` configuration file. 
Ensure that `-Drtlm.web.db=/opt/symetry/symetry-web.txt` is set in `/opt/jetty/start.ini`.
